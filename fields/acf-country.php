...
class AcfCountry extends acf_field {
	...
	*/
	public function validate_value( $valid, $value, $field, $input ) {
		...
		$countries = array_values( $this->get_countries() );
		if ( is_array( $value ) ) {
			if ( count( array_diff( $value, $countries ) ) !== 0 ) {
				...
				$valid = sprintf( _n( '%s is not valid a country', '%s are not valid countries', count( $value ), 'acf-country' ), implode( ', ', $value ) );
			}
			...
			if ( ! in_array( $value, $countries, true ) ) {
				...
				$valid = sprintf( __( '%s is not a valid country', 'acf-country' ), $value );
			}
		}
		return $valid;
	}
